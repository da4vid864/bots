...
/**
 * Activar Trial de 14 días (Sin tarjeta)
 */
async function activateProTrial(userEmail) {
    try {
        const subscription = await getOrCreateSubscription(userEmail);
        
        // Solo permitir trial si está en plan free y no ha tenido trial antes (opcional: agregar flag has_had_trial)
        ...
        console.log(`🚀 Trial activado para ${userEmail} hasta ${trialEndsAt}`);
        return result.rows[0];
    } catch (error) {
